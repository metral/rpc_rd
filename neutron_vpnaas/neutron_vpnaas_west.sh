...
PUBLIC_SUBNET_NAME=west-subnet
PRIVATE_SUBNET_NAME=mywest-subnet
...
sudo ip route add 172.24.4.225/32 via 172.24.4.232      # Add static route on the host to east br-ex interface via the host br-ex interface
sudo ip route add 172.24.4.226/32 via 172.24.4.225      # Add static route on the host to east neutron router gateway qr-<ID> interface via the east br-ex interface (which is via the host br-ex interface)

neutronrouter=`sudo ip netns | grep qrouter`    # Grab west neutron router name
sudo ip netns exec $neutronrouter ip route add 172.24.4.225/32 via 172.24.4.232     # Add static route on the neutron router to east br-ex interface via the host br-ex interface
sudo ip netns exec $neutronrouter ip route add 172.24.4.226/32 via 172.24.4.232     # Add static route on the neutron router to east neutron router gateway qr-<ID> interface via the host br-ex interface

neutronrouter_gateway=`sudo ip netns exec $neutronrouter ifconfig | grep qg | awk '{print $1}'`    # Grab west neutron router gateway name
...
neutron vpn-service-create --name myvpn --description "My vpn service" router1 mywest-subnet
neutron ipsec-site-connection-create --name conn_west --vpnservice-id myvpn --ikepolicy-id ikepolicy1 --ipsecpolicy-id ipsecpolicy1 --peer-address 172.24.4.226 --peer-id 172.24.4.226 --peer-cidr 10.1.0.0/24 --psk secret

echo ""
echo "SSH into VM & ping the east private subnet:"
echo "==========================================="
echo "$ ssh cirros@$FLOATING_IP"
echo "Password: cubswin:)"
echo "vm: $ ping 10.1.0.4"
